...
const STRATEGY_COLOR = "#22c55e";
const SPY_COLOR = "#f59e0b";
const SF_COLOR = "#a855f7";
...
export default function DmEquityCurveChart({ equityCurve, curveKey, curveLabel }) {
  if (!equityCurve || equityCurve.length < 2) return null;

  const keys = [curveKey, "spy", "sixtyForty"].filter((k) => equityCurve[0][k] != null);
  const colors = { [curveKey]: STRATEGY_COLOR, spy: SPY_COLOR, sixtyForty: SF_COLOR };
  const labels = { [curveKey]: curveLabel || curveKey.toUpperCase(), spy: "SPY B&H", sixtyForty: "60/40" };
  ...
  const xLabels = equityCurve
    ...
    .map((p) => ({ x: toX(equityCurve.indexOf(p)), label: p.date?.slice(0, 7) || "" }));
