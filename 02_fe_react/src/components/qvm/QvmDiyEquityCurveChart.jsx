...
const VARIANT_COLORS = {
  qvmEw: { primary: "#10b981", spy: "#f59e0b" },
  qvmMom: { primary: "#f43f5e", spy: "#f59e0b" },
};

const VARIANT_LABELS = {
  qvmEw: { primary: "QVM-EW (균등)", spy: "SPY B&H" },
  qvmMom: { primary: "QVM-MOM (모멘텀)", spy: "SPY B&H" },
};
...
export default function QvmDiyEquityCurveChart({ equityCurve, variant = "qvmEw" }) {
  if (!equityCurve || equityCurve.length < 2) return null;

  const colors = VARIANT_COLORS[variant] || VARIANT_COLORS.qvmEw;
  const labels = VARIANT_LABELS[variant] || VARIANT_LABELS.qvmEw;
  const COLORS = { [variant]: colors.primary, spy: colors.spy };
  const LABELS = { [variant]: labels.primary, spy: labels.spy };
  ...
  for (let i = 0; i <= 4; i++) {
    const v = minY + step * i;
    yTicks.push({ v, y: toY(v), label: v.toFixed(2) });
  }
