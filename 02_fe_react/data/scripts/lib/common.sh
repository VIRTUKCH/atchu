...
send_webhook() {
  ...
  local webhook_url="${DISCORD_ATCHU_DEV_LOG_WEBHOOK_URL:-}"
...
send_dev_trend_webhook() {
  local message="$1"
  local webhook_url="${DISCORD_ATCHU_DEV_TREND_SIGNAL_WEBHOOK_URL:-}"
  if [[ -z "${webhook_url}" ]]; then
    return 0
  fi
  local payload
...
enable_webhook_if_configured() {
  if [[ -n "${DISCORD_ATCHU_DEV_LOG_WEBHOOK_URL:-}" ]]; then
    WEBHOOK_ENABLED=1
  fi
}
