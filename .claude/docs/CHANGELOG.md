...
### 2026-04-16 (마케팅)
- **토스 종목토론방 게릴라마케팅 첫 성과**
  - 3일 만에 신규 유저 15명 유입 (링크 없이 텍스트 소개만으로)
  - 전략 유효성 검증 완료 — `01_종목토론방_게릴라마케팅.md` 전략 그대로 유지
...
- **ETF 풀 대규모 확장: 인버스 29개 + 정방향 13개 (총 42개 신규)**
  - `trading.json`: 인버스 ETF 29개 추가 (국가/지역 6, 섹터 12, 광역지수 2, 채권 3, 원자재 4, 기타 2)
    - 국가: YXI(중국), EWV(일본), EPV(유럽), EFZ(선진국), EUM(신흥국), BZQ(브라질)
    - 섹터: REW(기술), SEF(금융), DUG(에너지), RXD(헬스케어), BIS(바이오), SSG(반도체), SIJ(산업재), SMN(소재), SCC(임의소비재), SZK(필수소비재), SDP(유틸리티), REK(부동산)
    - 광역: DOG(다우), RWM(러셀)
    - 채권: TBF(장기국채), TBX(중기국채), SJB(하이일드)
    - 원자재: GLL(금), ZSL(은), SCO(원유), KOLD(천연가스)
    - 기타: EUO(유로), BITI(비트코인)
  - `country.json`: 정방향 국가 ETF 6개 추가 — EWG(독일), EWQ(프랑스), EWL(스위스), EWH(홍콩), EZA(남아공), KSA(사우디)
  - `sector.json`: 정방향 섹터 ETF 7개 추가 — XBI(바이오텍), ITA(방산), GDX(금광주), ICLN(클린에너지), XOP(유가스탐사), KRE(지역은행), XHB(주택건설)
  - 배율 원칙: -1x 우선, -1x 없으면 -2x 대체. 개별주 인버스 제외.
...
- **경기순환 섹터 로테이션: 국면 판단 로직 개선 (SPY 6M+IEF/SHY → 13612W)**
  ...
  - 개선: SPY 13612W 가중 모멘텀의 레벨(양/음) × 방향(상승/하락) → 균등 분포 (회복7%, 호황42%, 둔화32%, 침체19%)
  - IEF/SHY 의존 제거, SPY만으로 판단
  - CAGR 9.32% (SPY 8.2% 초과), 백테스트 기간 2000~2026 (314개월)
  - 프론트엔드: phaseIndicators 표시 업데이트, 2×2 그리드 축 재배치
...
- **Faber 섹터 모멘텀: XLC 제외 → 10개 섹터, 백테스트 20년+ 확장**
  ...
  - 백테스트: 84개월(7년) → 249개월(20년+), 2008 금융위기 포함
  - 결과: CAGR 8.94%, MDD -22.53% (SPY -50.78% 대비 절반 이하)
  - 페이지에 "유니버스 구성" 설명 섹션 추가 (XLC 제외 이유, VNQ 대체 근거)
