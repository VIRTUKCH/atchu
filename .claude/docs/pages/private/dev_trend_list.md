...
**일반 사용자에게 공개되지 않는 모든 티커**를 하나의 통합 리스트로 보여주는 개발자 전용 페이지.

> **원칙**: 공개 서비스(`/trend_list`)에서 숨겨진 티커는 이 페이지에서 전부 보인다.

| 카테고리 | 종목 수 | 비고 |
|---|---|---|
| S&P 500 개별주 | ~503 | 공개 서비스에 없음 |
| 공개 ETF | 85 | `/trend_list`에도 있음 (비교용으로 포함) |
| 레버리지·인버스 ETF (지수·섹터) | ~9 | `leverage.json` / `inverse.json` |
| 단일종목 레버리지·인버스 ETF | ~22 | `stock_leverage.json` / `stock_inverse.json` |

총 ~619개 (레버리지·인버스 합산 기준).
...
- "레버리지·인버스 ETF 중 지금 추세 진입한 건 뭐가 있지?"
- "TSLL이나 NVDL 지금 추세 안에 있나?"
...
| 순서 | 영역 | 설명 |
|------|------|------|
| 0 | 요약 바 | "전체 ~619개 중 {M}개 앗추 필터 통과" (레버리지·인버스 포함 전체 카운트) |
