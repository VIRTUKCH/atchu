...
**Balanced** 변형은 공격 모드에서 G12 유니버스 top 6에 균등 분산 투자한다.

### 포트폴리오 규칙

- **공격 모드**: G12 유니버스(12개 ETF) 중 상대 모멘텀 상위 6개 → 각 16.7% 배분
...
equity curve 키: `balanced`

---

## 데이터 출처

`baa_signal.json`의 `backtest.equityCurve[]` 배열에서 `balanced` 값 사용.

---

## 컴포넌트

기존 `BaaQuantPeekPage`를 `variant="balanced"` prop으로 호출. 공유 섹션(카나리아, 순위)은 동일하게 표시하되, 포트폴리오·수익률·히스토리는 Balanced만 표시.
