...
추세 조회 리스트의 ETF 카드(`index-card`)와 동일한 2열 그리드 패턴. 좌측에 포트폴리오+수익률, 우측에 백테스트 통계 박스(`index-ma-dl-row` 패턴).

```
PC (769px+):
┌──────────────────────────────────────────────────────────────┐
│ [BAA-A (Aggressive)]  G4 top 1 집중 투자                     │ head
│ [공격] 2026-02 월말 기준                                      │
├─────────────────────────┬────────────────────────────────────┤
│ 현재 포트폴리오          │ ┌──────────────────────────────┐  │
│ EFA 선진국 (EAFE) 100%  │ │ CAGR          +9.72%        │  │
│                          │ └──────────────────────────────┘  │
│ 기간별 수익률            │ ┌──────────────────────────────┐  │
│ ┌──────────┬──────────┐ │ │ MDD           -13.6%        │  │
│ │    1Y    │    6M    │ │ └──────────────────────────────┘  │
...
모바일 (768px 이하):
body가 1열로 스택. 좌측(포트폴리오+수익률) → 우측(통계 박스) 순서.
```
...
| 요소 | 설명 |
|------|------|
| **현재 포트폴리오** | 티커 + 한글명 + 비중%. BAA-A는 1줄, BAA-B는 6줄 |
| **기간별 수익률** | 2×2 그리드 박스 (1Y, 6M, 3M, 1M). 순수 USD 기준 |

**Right** (이 전략을 신뢰할 수 있나)
...
- **레이아웃**: 2열 × 2행 (4개 박스). 400px 이하에서도 2열 유지
...
#### 통계 박스 (`quant-stats-row`)

- `index-ma-dl-row` 패턴과 동일: `padding: 10px 14px`, `border-radius: 10px`
...
|------|-------------|----------------|
| `quant-card-body` | 2열 그리드 (1.2fr 1fr) | 1열 스택 |
| 수익률 그리드 | 2×2 | 2×2 (유지) |
