...
이격률(avgDiff200)이 필터 통과 개수(x/y)보다 중요한 수치다.
- **이격률**: 시장의 실제 강도를 보여주는 핵심 지표 — 크게 표시
- **통과 비율(%)**:섹터 전체 건강도 요약 — 중간 강조
- **통과 개수(x/y)**: 보조 참고용 — 괄호로 처리해 시각적으로 작게
...
| 섹터명 | 섹터 식별 | bold |
| 바 | 통과 비율 시각화 | 시각적 |
| 통과 비율% | 섹터 강도 요약 (공개 페이지와 공유) | bold, 강조 |
| 평균 이격률% | MA200 대비 평균 거리 — 핵심 수치 | bold, 컬러 |
| 통과 개수(x/y) | 분모·분자 참고용 | 괄호, 흐림 |

- 이격률 데이터 없을 때: 이격률 숨김 (null 처리)
- 양수(+): 초록, 음수(-): 빨강
...
| 요소 | 역할 | 강조 수준 |
|------|------|---------|
| 이름 | 서브섹터 식별 (flex:1, 말줄임) | 기본 |
| 평균 이격률% | 핵심 수치 | bold, 컬러 |
| 통과 개수(x/y) | 보조 참고 | 괄호, 흐림 (opacity 0.6) |
...
> **공개 페이지 공유**: MainMarketStatusGrid는 공개 마켓 오버뷰(/market_overview)와 공유됨.
> 공개 페이지의 ETF 데이터에는 avgDiff200이 없으므로 이격률 컬럼은 자동으로 숨겨짐 (null 조건).
> 통과 개수 괄호 포맷은 양쪽에 동일 적용.
...
- 서브섹터: 하단에 **최대 4열 그리드**로 표시. 각 서브섹터는 **compact chip** 형태
  - 칩 내 구성 (**1줄 가로**): `이름(flex:1, 말줄임) · 개수(작고 흐리게) · diff%(우측 고정, bold, 색상)`
  - 배경(`rgba(95,104,255,0.04)`) + 테두리(`1px solid`) + padding `5px 8px`
  - diff%: 양수 초록, 음수 빨강. `margin-left: auto`로 우측 고정
...
| 시도 | 구성 | 결과 |
|------|------|------|
| 2열 · 1줄 | 이름\|개수\|diff% 가로 | 셀이 넓어 이름/숫자 사이 공간 어색 |
| 3열 → 4열 · 1줄 | 동일 | 좁아졌지만 짧은 이름이 좌측에 덩그러니 |
| 4열 · 세로 2행 | 이름 위 / 개수+diff% 아래 | 카드 높이 증가, 정보 3개뿐인데 빈 공간 발생 |
| **4열 · compact chip (현재)** | 이름(flex:1) · 개수(흐리게) · diff%(우측 고정, 굵게) | ✅ 채택 |

**채택 근거:**
- 카드 형태 자체가 문제 — 3가지 정보(이름·개수·diff%)만으로 카드를 채우면 구조가 어떻든 빈 공간이 생김
- 해결책은 레이아웃 조정이 아니라 **높이 최소화** → compact chip으로 정보 밀도 확보
- diff%를 `margin-left: auto`로 우측에 고정해 수직 스캔 시 숫자끼리 정렬됨
- 개수는 이름 바로 뒤 작고 흐리게 → 시선을 방해하지 않으면서 필요 시 확인 가능
