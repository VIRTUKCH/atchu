...
- 테마 토글(다크/라이트) 3초 안에 5번 전환 → 상단 네비에 2개 탭 노출:
  - **"관리자 추세 조회"** → `/_stocks` 직행
  - **"관리자 시장 개요"** → `/_stocks_overview` 직행
...
- ~~섹터별 추세 강도 정렬~~ ✅
- ~~히트맵 GICS 섹터 그룹핑~~ ✅
- ~~sp500.json industry/subType/short_description 필드~~ ✅
- ~~반도체→기술, 바이오텍→헬스케어 통합~~ ✅
...
### 진행 중
- 리스트 페이지 URL 파라미터로 서브섹터 직접 진입 (`?sector=기술&sub=반도체`)
- 섹터 추세 강도 카드에서 서브섹터 클릭 → 리스트 페이지 서브섹터 필터 연동
