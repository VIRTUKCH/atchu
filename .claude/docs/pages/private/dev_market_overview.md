...
- 서브섹터: 하단에 **최대 4열 그리드**로 표시. 각 서브섹터는 독립된 박스(카드) 형태
  - 박스 내 구성 (**1줄 가로 배치**): `이름(flex:1, bold, 말줄임) | 개수(x/y) | diff%`
...
1줄 배치가 맞는 이유:
- 이름(좌, bold) → 개수(우) → diff%(우) 순서로 좌→우 자연스러운 시선 흐름
- 카드 높이가 최소화되어 섹터 전체를 스크롤 없이 조망 가능
- 개수와 diff%가 오른쪽에 고정되어 수직 스캔 시 숫자끼리 정렬됨
