...
- 서브섹터: 하단에 **2열 그리드**로 표시. 셀 1줄 구성: `이름(flex:1) | 개수 | diff%`
  - 이름: 굵게(700), 공간 부족 시 말줄임. 개수·diff: 오른쪽 고정(flex-shrink:0)
  - diff%: 양수 초록, 음수 빨강
...
각 서브섹터 내 정렬:
- **이격률 탭**: 진입 → 주의 → 하락, 각 그룹 내 이격률 내림차순
- **기간별 탭**: 수익률 내림차순
...
- 각 티커: 앗추 필터 상태 배지(진입/하락) + 티커명 + 기초자산
- 클릭 시 `/_dev_trend_list/:ticker` 상세로 이동
- 추세 진입 중인 것만 먼저 표시, 이탈·하락은 흐리게
