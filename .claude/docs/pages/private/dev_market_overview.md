...
- 서브섹터: 하단에 **최대 4열 그리드**로 표시. 각 서브섹터는 독립된 박스(카드) 형태
  - 박스 내 구성 (**세로 2행**): 이름(상단, bold) / 개수(하단 좌, 작고 흐리게) + diff%(하단, bold, 색상 강조)
  - 박스: 배경색(`rgba(95,104,255,0.04)`) + 테두리(`1px solid`)로 영역 구분
  - diff%: 양수 초록, 음수 빨강
- 클릭 → 아래 S&P 500 히트맵 섹션의 해당 섹터 위치로 자동 스크롤 (smooth scroll)
- ETF용 기존 4열 그리드는 유지 (subSectors 유무로 자동 분기)
- 모바일: 2열 유지 (PC: 4열)

**서브섹터 셀 레이아웃 선택 근거 (1줄 가로 배치)**

세로 스택(이름 위 / 숫자 아래) 방식을 시도했으나 기각. 이유:
- 카드 높이가 늘어나 정보 밀도가 떨어짐 — 서브섹터가 많은 섹터(5개)에서 섹션 전체 높이가 과도해짐
- 이름이 상단에 덩그러니 있고 숫자가 하단에 분리되어 시선 이동이 자연스럽지 않음
- 관리자 도구는 빠른 스캔이 목적 — 같은 행에서 이름·개수·diff%를 한 번에 읽는 게 더 효율적

세로 2행 배치가 맞는 이유:
- diff%가 이 페이지에서 가장 중요한 숫자 — 색상(초록/빨강) + bold로 시각적 주인공
- 개수(x/y)는 부가 정보 → 작고 흐리게 처리해 위계 명확
- 짧은 이름(전력, 포장, 은행)이 1줄에서 좌측 한쪽에만 있는 어색함 해소
- padding 7px으로 컴팩트하게 유지해 이전 기각 사유(높이 낭비) 해결
