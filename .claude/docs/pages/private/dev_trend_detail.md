...
| 값 | 조건 | 배지 | 색상 |
|---|------|------|------|
| `"full"` | price > MA50 > MA100 > MA200 | `정배열` | 초록 |
| `"partial"` | price > MA200 AND MA50 > MA200 | `부분 정배열` | 노랑 |
| `"none"` | 위 조건 모두 미충족 | (미표시) | — |
| `"reverse"` | MA200 > MA100 > MA50 > price | `역배열` | 빨강 |

위치: 앗추 필터 인디케이터 옆 (`.atchu-filter-indicator` 내부)
