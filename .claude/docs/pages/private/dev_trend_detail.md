...
- `crossingHistory.periods`에 `"full_align"` 포함 → 탭 자동 생성
- `crossingHistory.periodLabels["full_align"]` = `"정배열"`
- `crossingHistory.items`에서 `period === "full_align"` 필터링 → 거래 내역
- `crossingHistory.annualizedMap["full_align"]` → CAGR
- `crossingHistory.mddMap["full_align"]` → MDD
