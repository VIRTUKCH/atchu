...
**섹터별 추세 강도 카드**:
- 각 섹터 카드: 전체 종목 통과율 + 서브섹터별 미니 바 (소프트웨어 14/20, 반도체 13/19 ...)
- 클릭 → `/_stocks?sector=기술` 이동
...
- **섹터 필터**: GICS 11개 섹터 칩 (전체 / 기술 / 헬스케어 / ...)
- **서브섹터 필터**: 섹터 선택 시 해당 섹터의 서브섹터 칩이 추가 표시
...
2. ~~**히트맵 그룹핑 수정**: ETF 그룹핑 → GICS 섹터별 그룹핑~~ ✅
3. ~~**sp500.json에 industry 필드 추가**~~ ✅
4. **sp500.json에 subType 필드 추가**: industry → subType 매핑 적용 (503종목)
5. **type 필드 정리**: 반도체 → 기술, 바이오텍 → 헬스케어로 통합

### Phase 2: UI — 서브섹터 표시
6. **섹터 추세 강도 카드**: 서브섹터별 미니 바 추가
7. **히트맵**: 섹터 내 서브섹터 소제목 구분
8. **리스트 페이지**: 섹터 선택 시 서브섹터 필터 칩 표시

### Phase 3: 시각화 개선
9. 히트맵 시각화 방식 재검토 (트리맵, 아코디언 등)
10. 시장 폭 히스토리 차트 (날짜별 앗추 필터 통과율 추이)
