...
#### 섹터 분류 (13개 — GICS 11개 + 반도체/바이오텍 분리)

| 섹터 | 종목 수 | 원래 GICS | 분리 이유 |
|------|--------|----------|----------|
| 기술 | 52 | Information Technology | 소프트웨어, 하드웨어, IT서비스 (반도체 제외) |
| **반도체** | **19** | Information Technology | AI/데이터센터 테마, 기술 내에서 독립적 사이클 |
| 헬스케어 | 52 | Health Care | 제약, 의료기기, 헬스케어 서비스 (바이오텍 제외) |
| **바이오텍** | **8** | Health Care | 제약/의료기기와 다른 움직임, 고변동 |
| 산업재 | 79 | Industrials | 항공우주, 방산, 운송, 기계, 건설 |
| 금융 | 76 | Financials | 은행, 보험, 자산운용, 핀테크 |
| 임의소비재 | 48 | Consumer Discretionary | 유통, 자동차, 레저, 의류, 호텔 |
| 필수소비재 | 36 | Consumer Staples | 식품, 음료, 생활용품, 유통 |
| 유틸리티 | 31 | Utilities | 전력, 가스, 수도, 신재생 |
| 부동산 | 31 | Real Estate | 리츠, 부동산 개발 |
| 소재 | 26 | Materials | 화학, 건자재, 금속, 포장 |
| 통신 | 23 | Communication Services | 미디어, 인터넷, 통신, 게임 |
| 에너지 | 22 | Energy | 석유, 가스, 에너지 장비 |
...
반도체와 바이오텍만 분리. 나머지 127개 GICS Sub-Industry 세분화는 불필요 — 히트맵에서 개별 종목이 이미 보이기 때문.

- `sp500.json`에 `industry` 필드(GICS Sub-Industry)는 이미 추가됨 (127개 산업)
- `type` 필드에서 Semiconductor* → "반도체", Biotechnology → "바이오텍"으로 재분류
- 섹터 카드 클릭 → `/_stocks?sector=반도체` 등으로 이동, 필터 칩 자동 선택
