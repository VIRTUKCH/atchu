...
.claude/docs/
  pages/                 — 페이지별 기획 (타겟 사용자, 유입 경로, 보여줄 것, CTA). README는 이유(why), pages/*.md는 구체적 구현 설계(what/how)
    ...
    private/             — 비공개 개발자 전용 페이지 기획 (일반 사용자 노출 X)
      dev_market_overview.md — 개발자 마켓 뷰 (/_stocks_overview, 데이터 구조)
      dev_trend_list.md    — 개별주 추세 조회 (/_stocks) — 섹터/서브섹터 필터, 정렬
      dev_trend_detail.md  — 개별주 상세 (/_stocks/:ticker) — 차트 2선, 골든크로스 배지, 전략 비교
