...
## 개발자 전용 비공개 페이지
일반 사용자에게 노출되지 않는 비밀번호 보호 페이지.

### 진입 방법
다크모드/라이트모드 토글을 **3초 안에 5번** 전환하면 상단 네비게이션에 개발자 전용 탭이 나타난다 (세션 동안 유지). 개발자 탭에서 아래 페이지로 이동 가능.

### 페이지 목록
- `/_dev` — 개발자 대시보드
- `/_stocks` — 개별주(S&P 500) 추세 조회 리스트
- `/_stocks/:ticker` — 개별주 상세 (차트, 교차이력, CAGR/MDD)
- `/_stocks_overview` — 개별주 시장 개요 (섹터별 추세 강도, 히트맵, 최근 신호)

### 기술 구현
- 개별주 CSV는 빌드에 번들하지 않고 `public/csv_stock/`에 정적 에셋으로 배치 → 상세 페이지에서 `fetch()`로 on-demand 로드
- 테마 토글 연타 감지: `AppTopNav.jsx`의 `handleThemeToggle`에서 timestamp 배열로 3초 내 5회 감지
- 개발자 모드 활성화 시 `devTabs` 배열이 네비게이션에 추가됨
