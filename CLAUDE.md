...
- 웹 검색이 필요하다고 판단되면 사용자에게 묻지 않고 전역 스킬을 자동 실행
  - 심층·딥 리서치 요청 → `/research` (여러 라운드 검색, 교차 검증, 출처 품질 평가)
  - 간단·짧게 알아봐 달라는 요청 → `/search` (빠른 자동 검색, 확인 절차 없음)
...
.claude/docs/
  pages/                 — 페이지별 기획 (타겟 사용자, 유입 경로, 보여줄 것, CTA). README는 이유(why), pages/*.md는 구체적 구현 설계(what/how)
    ...
    private/             — 비공개 개발자 전용 페이지 기획 (일반 사용자 노출 X)
      dev_market_overview.md — 개발자 마켓 뷰 (/_dev_market_overview, 데이터 구조)
      dev_trend_list.md    — 개별주 추세 조회 (/_dev_trend_list) — 섹터/서브섹터 필터, 정렬
      dev_trend_detail.md  — 개별주 상세 (/_dev_trend_list/:ticker) — 차트 2선, 골든크로스 배지, 전략 비교
      dev_discord_alert.md — 개발자 전용 Discord 알림 (관리자 채널 — 파이프라인 상태, 레버리지·인버스 신호)
      dev_quant/            — 퀀트 엿보기 (/_dev_quant) — 퀀트 전략 허브
        dev_quant_index.md   — 허브 공통 (페이지 구조, 라우트, UI 가이드)
        dev_quant_baa_a.md   — BAA-A Aggressive (G4 top 1 집중)
        dev_quant_baa_b.md   — BAA-B Balanced (G12 top 6 분산)
        dev_quant_sector.md  — 섹터 로테이션 전략 (예정)
...
기획 상세: `.claude/docs/pages/private/dev_market_overview.md`

### 진입 방법
다크모드/라이트모드 토글을 **3초 안에 5번** 전환 → 상단 네비에 "관리자 추세 조회" / "관리자 시장 개요" 탭 직접 노출

### 페이지 목록
- `/_dev` — 관리자 허브 (개별주 페이지 진입점)
- `/_dev_market_overview` — 개별주 시장 개요 (시장 폭, 섹터별 추세 강도, 히트맵, 최근 신호)
- `/_dev_trend_list` — 개별주 추세 조회 리스트 (섹터 필터, 검색, 정렬)
- `/_dev_trend_list/:ticker` — 개별주 상세 (차트, 교차이력, CAGR/MDD)
...
## 퀀트 엿보기 (비공개)
15개 퀀트 전략(BAA, HAA, 듀얼 모멘텀, 경기순환, 리스크 패리티, CTA, 멀티팩터 QVM 등)의 백테스트·시그널 확인 도구.
앗추 제품과 분리된 개인 투자 도구. 일반 사용자에게 절대 노출되지 않는다. README에 언급하지 않는다.
기획 상세: `.claude/docs/pages/private/dev_quant/dev_quant_index.md`

### 진입 방법
개발자 마켓 뷰와 동일. 다크모드/라이트모드 토글을 **3초 안에 5번** 전환 → 상단 네비에 "퀀트 엿보기" 탭 노출

### 페이지 목록
- `/_dev_quant` — 퀀트 허브 (전략 카드 리스트, 수익률·통계 비교)
- `/_dev_quant/:strategy` — 전략 상세 (포트폴리오, 리밸런싱 시그널, 백테스트 차트)
